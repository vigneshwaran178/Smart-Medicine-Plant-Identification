...
def add_plant():
    ...
    if request.method=='POST':
        ...
        mycursor.execute("SELECT max(id)+1 FROM plant_uses")
        maxid = mycursor.fetchone()[0]
        if maxid is None:
            maxid=1
                
        sql = "INSERT INTO plant_uses(id,plant,uses) VALUES (%s, %s, %s)"
        val = (maxid,plant,uses)
        mycursor.execute(sql,val)
        mydb.commit()
        return redirect(url_for('add_plant'))

    mycursor.execute('SELECT * FROM plant_uses')
...
def test_pro3():
    ...
    cursor = mydb.cursor()
    cursor.execute('SELECT count(*) FROM plant_uses where plant=%s',(ts,))
    cnt = cursor.fetchone()[0]
    st=""
    data3=[]
    if cnt>0:
        st="1"
        cursor.execute('SELECT * FROM plant_uses where plant=%s',(ts,))
        data3 = cursor.fetchall()

        
    return render_template('test_pro3.html',msg=msg,fn=fn,ts=ts,act=act,data3=data3,st=st)
